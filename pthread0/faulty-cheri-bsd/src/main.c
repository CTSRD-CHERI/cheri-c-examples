//cheri compiler errors/warnings
//pthread0
//#include <cheri.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <malloc.h>
#include <pthread.h>
/*
 pthread0 
*/

typedef unsigned long uint64_t;
typedef unsigned char uint8_t;

extern int printf(const char *, ...);

#if 1
//char mem[0x1000];
int w=0;
char * my_malloc(int sz)
{
   //char *m = &mem[w];	
   uint64_t * p = (uint64_t *) malloc(sz+8);
   p[0] = sz;
   return (char*) &p[1];  
   //w= w+sz;
   //return m;
}
#else 
extern char * my_malloc(int sz);
#endif
/***********************************************/
pthread_mutex_t * ctest1(void)
{

pthread_mutex_t * pmutex = (pthread_mutex_t *)my_malloc(sizeof (*pmutex)) ; 
return pmutex ;
}

char tmp[]="hello";

/*******************************************/
int main(int argc, char * argv[])
{
	int i;
for (i=0;i<1;i++)
{
	printf("\n... try %d..\n", i);
	pthread_mutex_t *pm = ctest1();

        //on cheribsd, pthread_mutex_t is just a void *
        // we will set *pm to some pointer(capability) to show that its storage is misaligned
        //by trying to access
        *pm = (pthread_mutex_t) tmp ;
        *((char*)*pm) = 'H';  
       
        //note these will pass on cheribsd because they seem to be nops until actual pthreads are created.. 
        printf(" %ld %#p\n",sizeof(*pm), pm);
	int res=pthread_mutex_init(pm,0);
	printf("after pt_mutext_init %d\n",res);

	res = pthread_mutex_destroy(pm);
	printf("after pt_mutex_destroy %d\n",res);
}
}
